...
egrep -i "\"filter\"\s*\:" $ARGS | sed -r "s/.*\"filter\"\s*\:\s*\"([a-z])\"\,/\1/g" > band.list
...
#    echo -e "\noverlapnum = ${overlapnum} , overlapnite = ${overlapnite} , explength = $explength, teff = $teff"
	...
	if [ $nfiles -ge 59 ] ; then
	    ./getcorners.sh $overlapnum $rpnum /pnfs/des/scratch/gw/exp/${overlapnite}/${overlapnum}
	    if [ $? -ne 0 ] ; then 
...
###    # determine year setting for SE processing	
###    # need to implement here handling of different "epochs" within the same year
###    if [ $EXPNUM -lt 320000 ]; then
###	YEAR=y1
###    elif [ $EXPNUM -lt 600000 ]; then
###	YEAR=y2
###    fi
###
...
###ho "ifdh cp -D $DOTOUTFILES \$TOPDIR_WSTEMPLATES/pairs/" > $templatecopyfile
###
###copy over the desservices.ini file, if not already there
### [ -e /pnfs/des/scratch/gw/db-tools/desservices.ini ] ; then
...
### [ -e /pnfs/des/scratch/marcelle/run_SEproc.py ] ; then
###  $RMCMD  /pnfs/des/scratch/marcelle/run_SEproc.py
###
###OPYDCMD run_SEproc.py /pnfs/des/scratch/marcelle
###HMOD /pnfs/des/scratch/marcelle/run_SEproc.py
###
###copy over the run_SEproc_noSkySub.py file. 
### [ -e /pnfs/des/scratch/marcelle/run_SEproc_noSkySub.py ] ; then
###  $RMCMD  /pnfs/des/scratch/marcelle/run_SEproc_noSkySub.py
###
###OPYDCMD run_SEproc_noSkySub.py /pnfs/des/scratch/marcelle
###HMOD /pnfs/des/scratch/marcelle/run_SEproc_noSkySub.py
###
...
###ho "<serial>" >> $outfile
###ho "jobsub -n --group=des --OS=SL6  --resource-provides=usage_model=${RESOURCES} --memory=500MB --disk=100MB --expected-lifetime=600s $JOBSUB_OPTS file://dummyjob.sh" >> $outfile
###ho "</serial>" >> $outfile
###
...
###ho "</parallel>" >> $outfile
###
###ho "run RUN_DIFFIMG_PIPELINE.pl with no nodelist to set up the directory structure and make the scripts"
###
##### Now we run RUN_DIFFIMG_PIPELINE.pl with no nodelist to set up the directory structure and make the scripts
###
###xport DIFFIMG_DIR=/data/des40.b/data/kherner/Diffimg-devel/diffimg-trunk
###xport PATH=`echo $PATH | sed -e s#\/cvmfs\/des.opensciencegrid.org\/eeups\/fnaleups\/Linux64\/diffimg\/gwdevel3#\/data\/des40.b\/data/kherner\/Diffimg-devel\/diffimg-trunk#`
###xport DIFFIMG_DIR=/data/des41.a/data/marcelle/diffimg/DiffImg-trunk
###xport PATH=`echo $PATH | sed -e s#\/cvmfs\/des.opensciencegrid.org\/eeups\/fnaleups\/Linux64\/diffimg\/gwdevel8#\/data\/des41.a\/data/marcelle\/diffimg\/DiffImg-trunk#`
###
###port DES_SERVICES=~/.desservices.ini
###port DES_DB_SECTION=db-sn-test
###port SCAMP_CATALOG_DIR=/cvmfs/des.opensciencegrid.org/fnal/SNscampCatalog
###port AUTOSCAN_PYTHON=$PYTHON_DIR/bin/python
###
###his gets the fake DB version file
#### make sure that this is still there!!! if not check des41
###port DES_ROOT=/data/des20.b/data/SNDATA_ROOT/INTERNAL/DES
###
###port TOPDIR_SNFORCEPHOTO_IMAGES=data/DESSN_PIPELINE/SNFORCE/IMAGES
###port TOPDIR_SNFORCEPHOTO_OUTPUT=data/DESSN_PIPELINE/SNFORCE/OUTPUT
###port TOPDIR_DATAFILES_PUBLIC=data/DESSN_PIPELINE/SNFORCE/DATAFILES_TEST
###port TOPDIR_TEMPLATES=/data/des30.a/data/WSTemplates
###
###ELD_TILING=$( egrep "^\s?${EXPNUM}" exposures.list | sed -e 's/.*DES.*hex //' -e 's/tiling//' -e s/\"// )
###
############# check and do something special if the word hex is not present ############
###
### [ `echo $FIELD_TILING | awk '{print $1}'` == $EXPNUM ]; then
###In this case field will be GW(RA*10)(DEC*10) with either + or - for the dec.   and the tiling will be 1
###
###ho "FIELD is currently set to the exposure number. This is mostly likely because the image doesn't have \"hex\" in the description. Setting FIELD according to new scheme."
###ageline=$(egrep "^\s?${EXPNUM}" exposures_${BAND}.list | awk '{print $4,$5}' )
###ARCHRA=`echo $imageline | cut -d " " -f 1`
###ARCHDEC=`echo $imageline | cut -d " " -f 2`
###10=$(echo "${SEARCHRA}*10" | bc | cut -d "." -f 1)
###C10=$(echo "$SEARCHDEC * 10" | bc | cut -d "." -f 1)
### [ $DEC10 -ge 0 ]; then 
###  DEC10="+${DEC10}"
###
###ELD="GW${RA10}${DEC10}"
###LING=1
###ELD_TILING="${FIELD} ${TILING}"
###eck_header
###
########## 
###
###se
###ELD=$( echo $FIELD_TILING | cut -d " " -f 1)
###LING=$( echo $FIELD_TILING | cut -d " " -f 2)
###ho "FIELD = $FIELD"
###ho "TILING = $TILING"
### [ "x${TILING}" == "x" ]; then
...
###
###sed -e s/THENITE/$NITE/ -e s/THEBAND/${BAND}/ -e s/THEEXP/${EXPNUM}/ -e s/THEFIELD/${FIELD}/ -e s/THETILE/${TILING}/ -e s/CCD2DIGIT/\$CCD/ INTERNAL_INFO_TEMPLATE.DAT > INTERNAL_INFO_${EXPNUM}_tile${TILING}.DAT
###
...
###	sed -i -e '/ln -sf/ s/ln -sf/ifdh cp/' -e '/ifdh/a ln -sf '$newlink' JOBDIR' ./${procnum}/${BAND}_`printf %02d ${iccd}`/RUN01_expose_prepData
###    fi
###done
###
###
...
###$COPYCMD mytemp_${EXPNUM}/KH_diff_RADEC.list1  /pnfs/des/scratch/gw/exp/${NITE}/${EXPNUM}/WS_diff.list
###
